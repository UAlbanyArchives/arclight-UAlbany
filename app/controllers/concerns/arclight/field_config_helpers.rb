...
module Arclight
  ...
  module FieldConfigHelpers
	...
	helper_method :link_to_creator_facet
    ...
    def link_to_subject_facet(args)
      options = args[:config]&.separator_options || {}
      values = args[:value] || []
      ...
      values.map do |value|
        view_context.link_to(
          value,
          view_context.search_action_path(f: { access_subjects_ssim: [value] })
        )
      end.to_sentence(options).html_safe
    end

    def link_to_creator_facet(args)
      options = args[:config]&.separator_options || {}
      values = args[:value] || []
      ...
      values.map do |value|
        view_context.link_to(
          value,
          view_context.search_action_path(f: { creator_ssim: [value] })
        )
      end.to_sentence(options).html_safe
    end

    def link_to_places_facet(args)
      options = args[:config]&.separator_options || {}
      values = args[:value] || []
      ...
      values.map do |value|
        view_context.link_to(
          value,
          view_context.search_action_path(f: { places_ssim: [value] })
        )
      end.to_sentence(options).html_safe
    end
